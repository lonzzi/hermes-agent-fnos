...
自动检查覆盖包结构、生命周期脚本语法、凭据校验、升级保留数据、重复发布判定、官方多架构镜像、fnpack 打包及 amd64 控制台登录。ARM64 仅检查上游镜像是否存在，尚未在 ARM64 runner 做启动测试。

**尚未完成飞牛实机验收**：最低 fnOS 版本、首次安装、桌面入口、运行状态、停止 / 重启、升级及卸载保留数据，需要在真实 NAS 上确认。不要将自动构建通过理解为这些项目均已验证。
