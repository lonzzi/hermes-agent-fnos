...
- 控制台使用安装时生成的随机签名密钥，并将登录会话保留一年；首次登录后，从同一浏览器的 fnOS 桌面再次打开通常无需重新输入密码。清除浏览器 Cookie、更换浏览器或主动退出后仍需登录。
...
- FPK 升级脚本不重写用户名或密码，不删除应用数据；旧版升级时只补充随机会话签名密钥和一年会话时长。升级前停止应用并备份上述两个目录；涉及数据库迁移的版本不能保证直接降级，回退需恢复备份。
...
**2026.9.14.3 已通过 ARM64 飞牛实机验证**：从 2026.9.14.2 原地升级成功，升级前后的控制台凭据、Hermes 配置和认证文件摘要一致；桌面图标可见，点击后在 fnOS 桌面内嵌窗口打开，不新增浏览器标签。全新安装、运行状态、停止 / 重新启动、Web 控制台访问、正确及错误密码登录也已通过。安装环境为 ARM64、内核 6.18.18.c1090-trim、appcenter-cli 1.0.1。最低 fnOS 版本、卸载保留数据和其他架构实机尚未验证。详见 [实机验证记录](docs/nas-validation.md)。
